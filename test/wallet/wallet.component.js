...
describe('wallet component', () => {
	...
	it('renders sen send button when senfund balance is zero', () => {
		const walletComponent = shallow(<Wallet synced confirmedbalance="10" unconfirmedbalance="1" />)
	...
	it('does not render show send prompt unless showSendView', () => {
		const walletComponent = shallow(<Wallet synced showSendView={false} confirmedbalance="10" unconfirmedbalance="1" actions={testActions} />)
		expect(walletComponent.contains(<SendView />)).to.be.false
	})
	it('renders show send prompt when showSendView', () => {
		const walletComponent = shallow(<Wallet synced showSendView confirmedbalance="10" unconfirmedbalance="1" actions={testActions} />)
		expect(walletComponent.contains(<SendView />)).to.be.true
	})
	it('does not render show receive prompt unless showReceiveView', () => {
		const walletComponent = shallow(<Wallet synced showReceiveView={false} confirmedbalance="10" unconfirmedbalance="1" actions={testActions} />)
		expect(walletComponent.contains(<ReceiveView />)).to.be.false
	})
	it('renders show receive prompt when showReceiveView', () => {
		const walletComponent = shallow(<Wallet synced showReceiveView confirmedbalance="10" unconfirmedbalance="1" actions={testActions} />)
