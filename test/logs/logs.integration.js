...
describe('logs plugin', () => {
	const sentientdir = SentientAPI.config.sentientd.datadir
	...
	it('starts with consensus and gateway logs shown', () => {
		expect(rootComponent.find('LogView').props().logText).to.equal(parseLogs(sentientdir, 50000, ['gateway.log', 'consensus.log']))
	})
	it('filters log view text when filter button is clicked', async () => {
		...
		for (let i = 0; i < filterControlNodes.length; i++) {
			...
			node.simulate('click')
			await sleep(50)
			expect(rootComponent.find('LogView').props().logText).to.equal(parseLogs(sentientdir, 50000, filterFilters))
		}
	})
	it('deselects filter controls when selected and shift-clicked', async () => {
		...
		expect(rootComponent.find('LogView').props().logText).to.equal(parseLogs(sentientdir, 50000, expectedFilters))
	})
})
